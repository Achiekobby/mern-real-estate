...
export default function EditListing() {
  ...
  const getListing = async () => {
    try {
      const res = await fetch(`/api/user/listing/show/${id}`, {
        method: "GET",
  ...
  //* using an effect hook to fetch the details of the listing to be edited
  useEffect(() => {
    getListing();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/user/listing/update/${id}`, {
        method: "POST",
        headers: {
        ...
        <main className="p-3 max-w-4xl mx-auto">
          ...
          <form className="flex flex-col sm:flex-row gap-4" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col flex-1 gap-4">
              ...
              >
                {submitLoading ? "Loading" : "Update Listing"}
              </button>
